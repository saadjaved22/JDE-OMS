({
  ...
  checkIfInvalidEmail: function (component, email) {
    ...
    if (
      !$A.util.isUndefinedOrNull(this.searchInList(Object.values(IEL), email))
    ) {
      component.set(
        "v.errorMessage",
        "E-mailaddressen die beginnen met info@, admin@, etc. worden niet geaccepteerd. Voer alstublieft een persoonlijk e-mailadres in."
      );
      component.set("v.showError", true);
      result = true;
    }
  ...
  checkEmailConfirmMatching: function (component, email, confirmEmail) {
    ...
    if (confirmEmail === email) {
      ...
      component.set(
        "v.errorMessage",
        "De opgegeven e-mailadressen komen niet overeen."
      );
      component.set("v.showError", true);
      component.set("v.submitButtonLoading", false);
  ...
  searchInList: function (stringList, keyword) {
    return stringList.find(
      (testing) => keyword.toLowerCase().search(testing.toLowerCase()) > -1
    );
  }
});
